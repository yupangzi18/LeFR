    ...
    void initialize(const Context &context) {
        ...
        for (auto partitionID = 0u; partitionID < partitionNum; partitionID++) {
            ...
            tbl_ycsb_vec.push_back(
                std::make_unique<Table<9973, ycsb::key, ycsb::value>>(ycsbTableID, partitionID));
        }
        ...
        initTables(
            "ycsb", [&context, this](std::size_t partitionID) { ycsbInit(context, partitionID); },
...
private:
    void ycsbInit(const Context &context, std::size_t partitionID) {
        ...
        if (context.strategy == PartitionStrategy::RANGE) {
            ...
            for (auto i = partitionID * keysPerPartition; i < (partitionID + 1) * keysPerPartition;
                 i++) {
