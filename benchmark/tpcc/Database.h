...
public:
    ...
    void initTables(const std::string &name, InitFunc initFunc, std::size_t partitionNum,
                    std::size_t threadsNum, Partitioner *partitioner) {
    ...
    void initialize(const Context &context) {
        ...
        auto partitioner = PartitionerFactory::create_partitioner(
            context.partitioner, coordinator_id, context.coordinator_num, partitionNum);
        ...
        for (auto partitionID = 0u; partitionID < partitionNum; partitionID++) {
            ...
            tbl_warehouse_vec.push_back(
                std::make_unique<Table<997, warehouse::key, warehouse::value>>(warehouseTableID,
                                                                               partitionID));
            auto districtTableID = district::tableID;
            tbl_district_vec.push_back(std::make_unique<Table<997, district::key, district::value>>(
                districtTableID, partitionID));
            auto customerTableID = customer::tableID;
            tbl_customer_vec.push_back(std::make_unique<Table<997, customer::key, customer::value>>(
                customerTableID, partitionID));
            auto customerNameIdxTableID = customer_name_idx::tableID;
            tbl_customer_name_idx_vec.push_back(
                std::make_unique<Table<997, customer_name_idx::key, customer_name_idx::value>>(
                    customerNameIdxTableID, partitionID));
            auto historyTableID = history::tableID;
            tbl_history_vec.push_back(std::make_unique<Table<997, history::key, history::value>>(
                historyTableID, partitionID));
            auto newOrderTableID = new_order::tableID;
            tbl_new_order_vec.push_back(
                std::make_unique<Table<997, new_order::key, new_order::value>>(newOrderTableID,
                                                                               partitionID));
            auto orderTableID = order::tableID;
            tbl_order_vec.push_back(std::make_unique<Table<997, order::key, order::value>>(

                orderTableID, partitionID));
            auto orderLineTableID = order_line::tableID;
            tbl_order_line_vec.push_back(
                std::make_unique<Table<997, order_line::key, order_line::value>>(orderLineTableID,
                                                                                 partitionID));
            auto stockTableID = stock::tableID;
            tbl_stock_vec.push_back(

                std::make_unique<Table<997, stock::key, stock::value>>(stockTableID, partitionID));
        }
        ...
        std::transform(tbl_order_vec.begin(), tbl_order_vec.end(), std::back_inserter(tbl_vecs[6]),
                       tFunc);
        std::transform(tbl_order_line_vec.begin(), tbl_order_line_vec.end(),
                       std::back_inserter(tbl_vecs[7]), tFunc);
        std::transform(tbl_item_vec.begin(), tbl_item_vec.end(), std::back_inserter(tbl_vecs[8]),
                       tFunc);
        std::transform(tbl_stock_vec.begin(), tbl_stock_vec.end(), std::back_inserter(tbl_vecs[9]),
                       tFunc);
        ...
        initTables(
            "warehouse", [this](std::size_t partitionID) { warehouseInit(partitionID); },
            partitionNum, threadsNum, partitioner.get());
        initTables(
            "district", [this](std::size_t partitionID) { districtInit(partitionID); },
            partitionNum, threadsNum, partitioner.get());
        initTables(
            "customer", [this](std::size_t partitionID) { customerInit(partitionID); },
            partitionNum, threadsNum, partitioner.get());
        initTables(
            "customer_name_idx",
            [this](std::size_t partitionID) { customerNameIdxInit(partitionID); }, partitionNum,
            threadsNum, partitioner.get());
        /*
        initTables("history",
        ...
        */
        initTables(
            "order", [this](std::size_t partitionID) { orderInit(partitionID); }, partitionNum,
            threadsNum, partitioner.get());
        /*
        ...
        */
        ...
        initTables(
            "stock", [this](std::size_t partitionID) { stockInit(partitionID); }, partitionNum,
            threadsNum, partitioner.get());
    }
    ...
    void apply_operation(const Operation &operation) {
        ...
        if (is_neworder) {
            ...
            while (dec.size() > 0) {
                ...
                dec >> stock_value.S_QUANTITY >> stock_value.S_YTD >> stock_value.S_ORDER_CNT >>
                    stock_value.S_REMOTE_CNT;
            }
        } else {
            {
                // warehouse
...
private:
    ...
    void customerNameIdxInit(std::size_t partitionID) {
        ...
        std::unordered_map<FixedString<16>, std::vector<std::pair<FixedString<16>, int32_t>>>
