...
#define NAMESPACE_FIELDS(x) x(lefr) x(tpcc)

#define WAREHOUSE_KEY_FIELDS(x, y) x(int32_t, W_ID)
#define WAREHOUSE_VALUE_FIELDS(x, y)                                                         \
    x(FixedString<10>, W_NAME) y(FixedString<20>, W_STREET_1) y(FixedString<20>, W_STREET_2) \
        y(FixedString<20>, W_CITY) y(FixedString<2>, W_STATE) y(FixedString<9>, W_ZIP)       \
            y(float, W_TAX) y(float, W_YTD)
...
#define DISTRICT_VALUE_FIELDS(x, y)                                                          \
    x(FixedString<10>, D_NAME) y(FixedString<20>, D_STREET_1) y(FixedString<20>, D_STREET_2) \
        y(FixedString<20>, D_CITY) y(FixedString<2>, D_STATE) y(FixedString<9>, D_ZIP)       \
            y(float, D_TAX) y(float, D_YTD) y(int32_t, D_NEXT_O_ID)
...
#define CUSTOMER_VALUE_FIELDS(x, y)                                                              \
    x(FixedString<16>, C_FIRST) y(FixedString<2>, C_MIDDLE) y(FixedString<16>, C_LAST)           \
        y(FixedString<20>, C_STREET_1) y(FixedString<20>, C_STREET_2) y(FixedString<20>, C_CITY) \
            y(FixedString<2>, C_STATE) y(FixedString<9>, C_ZIP) y(FixedString<16>, C_PHONE)      \
                y(uint64_t, C_SINCE) y(FixedString<2>, C_CREDIT) y(float, C_CREDIT_LIM)          \
                    y(float, C_DISCOUNT) y(float, C_BALANCE) y(float, C_YTD_PAYMENT)             \
...
#define HISTORY_KEY_FIELDS(x, y)                                                    \
    x(int32_t, H_W_ID) y(int32_t, H_D_ID) y(int32_t, H_C_W_ID) y(int32_t, H_C_D_ID) \
        y(int32_t, H_C_ID) y(uint64_t, H_DATE)
...
#define ORDER_VALUE_FIELDS(x, y)                                                         \
    x(float, O_C_ID) y(uint64_t, O_ENTRY_D) y(int32_t, O_CARRIER_ID) y(int8_t, O_OL_CNT) \
        y(bool, O_ALL_LOCAL)

DO_STRUCT(order, ORDER_KEY_FIELDS, ORDER_VALUE_FIELDS, NAMESPACE_FIELDS)

#define ORDER_LINE_KEY_FIELDS(x, y) \
    x(int32_t, OL_W_ID) y(int32_t, OL_D_ID) y(int32_t, OL_O_ID) y(int8_t, OL_NUMBER)
#define ORDER_LINE_VALUE_FIELDS(x, y)                                         \
    x(int32_t, OL_I_ID) y(int32_t, OL_SUPPLY_W_ID) y(uint64_t, OL_DELIVERY_D) \
        y(int8_t, OL_QUANTITY) y(float, OL_AMOUNT) y(FixedString<24>, OL_DIST_INFO)
...
#define STOCK_VALUE_FIELDS(x, y)                                                                  \
    x(int16_t, S_QUANTITY) y(FixedString<24>, S_DIST_01) y(FixedString<24>, S_DIST_02)            \
        y(FixedString<24>, S_DIST_03) y(FixedString<24>, S_DIST_04) y(FixedString<24>, S_DIST_05) \
            y(FixedString<24>, S_DIST_06) y(FixedString<24>, S_DIST_07)                           \
                y(FixedString<24>, S_DIST_08) y(FixedString<24>, S_DIST_09)                       \
                    y(FixedString<24>, S_DIST_10) y(float, S_YTD) y(int32_t, S_ORDER_CNT)         \
                        y(int32_t, S_REMOTE_CNT) y(FixedString<50>, S_DATA)
...
public:
    std::size_t operator()(StringPiece str, tpcc::customer::value &result) const {
        ...
        std::size_t sz_ytd_payment =
            Deserializer<decltype(result.C_YTD_PAYMENT)>()(str, result.C_YTD_PAYMENT);
        str.remove_prefix(sz_ytd_payment);
        std::size_t sz_payment_cnt =
            Deserializer<decltype(result.C_PAYMENT_CNT)>()(str, result.C_PAYMENT_CNT);
