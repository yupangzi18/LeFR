...
for delay in delays:
  for i in range(3):
    cmd = get_cmd(n, i)
    print('./bench_tpcc  --id=%d --servers="%s" --protocol=SiloGC --partition_num=%d --threads=12 --partitioner=pb --group_time=100 --delay=%d --query=mixed --neworder_dist=10 --payment_dist=15' % (id, cmd, 48, delay))  