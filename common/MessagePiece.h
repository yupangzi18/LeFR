 ...
 *
 * | Message type (7 => 128) | Message length (12 => 4096) | table id (4 => 16)
 * | partition id (9 => 512) |
 *
...
public:
    ...
    uint32_t get_table_id() const { return (get_header() >> TABLE_ID_OFFSET) & TABLE_ID_MASK; }
...
private:
    uint32_t get_header() const { return *reinterpret_cast<const uint32_t *>(stringPiece.data()); }
...
public:
    ...
    static uint32_t construct_message_piece_header(uint32_t message_type, uint32_t message_length,
                                                   std::size_t table_id, std::size_t partition_id) {
        DCHECK(message_type < (1u << 7));
        DCHECK(message_length < (1u << 12));
        DCHECK(table_id < (1u << 4));
        DCHECK(partition_id < (1u << 9));
...
public:
    ...
    static constexpr uint32_t TABLE_ID_MASK = 0xf;
    static constexpr uint32_t TABLE_ID_OFFSET = 9;
    static constexpr uint32_t PARTITION_ID_MASK = 0x1ff;
