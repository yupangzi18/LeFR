...
public:
    ...
    static uint64_t read(const std::tuple<MetaDataType *, void *> &row, void *dest,
                         std::size_t size) {
    ...
    static bool is_locked(uint64_t value) { return (value >> LOCK_BIT_OFFSET) & LOCK_BIT_MASK; }
    ...
    static void unlock(std::atomic<uint64_t> &a, uint64_t newValue) {
        ...
        newValue = remove_lock_bit(newValue);
