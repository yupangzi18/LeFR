...
#include "protocol/GPAC/GPACMessage.h"
...
public:
    ...
    using MessageType = GPACMessage;
    using TransactionType = GPACTransaction<DatabaseType>;

    using MessageFactoryType = GPACMessageFactory;
    using MessageHandlerType = GPACMessageHandler<DatabaseType>;

    GPAC(DatabaseType &db, const ContextType &context, Partitioner &partitioner,
    ...
    uint32_t pass_number() { return partitioner.replica_num() / 2 + 1; }
...
private:
    void backup_decision(TransactionType &txn, uint64_t commit_tid,
        ...
        while (txn.pendingResponses > 0) {
            ...
            ftCount = 0;
            for (auto partition : txn.ft_partitionSet) {
                if (partition.second >= pass_number()) {
                    ftCount += 1;
                }
            }
    ...
    bool validate_all_read_set(TransactionType &txn,
        ...
        for (auto i = 0u; i < partitioner.total_coordinators(); i++) {
            if (i == context.coordinator_id) {
                ...
                // record the local validation result
