...
public:
    ...
    Executor(std::size_t coordinator_id, std::size_t id, DatabaseType &db,
             ...
             std::atomic<uint32_t> &n_committing_txns, std::atomic<uint32_t> &n_redo_txns,
        ...
        for (;;) {
            ...
            do {
                ...
                if (!partitioner.is_backup()) {
                    ...
                    auto result =
                        transaction->execute(id, sync_messages, faultFlag, n_committing_txns,
    ...
    void onExit() override {
        LOG(INFO) << "Worker " << id << " latency: " << commit_latency.nth(50) << " us (50%) "
                  << commit_latency.nth(75) << " us (75%) " << commit_latency.nth(95)
                  << " us (95%) " << commit_latency.nth(99)
                  << " us (99%). write latency: " << write_latency.nth(50) << " us (50%) "
                  << write_latency.nth(75) << " us (75%) " << write_latency.nth(95) << " us (95%) "
                  << write_latency.nth(99)
                  << " us (99%). dist txn latency: " << dist_latency.nth(50) << " us (50%) "
                  << dist_latency.nth(75) << " us (75%) " << dist_latency.nth(95) << " us (95%) "
                  << dist_latency.nth(99)
    ...
    std::size_t process_request() {
        ...
        while (!in_queue.empty()) {
            ...
            for (auto it = message->begin(); it != message->end(); it++) {
                ...
                messageHandlers[type](messagePiece, *sync_messages[message->get_source_node_id()],
...
protected:
    ...
    std::vector<std::function<void(MessagePiece, Message &, ITable &, TransactionType *)>>
        messageHandlers;
    std::vector<std::size_t> message_stats, message_sizes;
    LockfreeQueue<Message *> in_queue, out_queue;
    std::unordered_map<uint32_t, std::unordered_set<int32_t>> &fault_partitions;
};
}  // namespace group_commit
