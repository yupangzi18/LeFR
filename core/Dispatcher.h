...
public:
    IncomingDispatcher(std::size_t id, std::size_t group_id, std::size_t io_thread_num,
                       std::vector<Socket> &sockets,
                       const std::vector<std::shared_ptr<Worker>> &workers,
                       LockfreeQueue<Message *> &coordinator_queue, std::atomic<bool> &stopFlag)
        : id(id),
          group_id(group_id),
    ...
    void start() {
        ...
        LOG(INFO) << "Incoming Dispatcher started, numCoordinators = " << numCoordinators
                  << ", numWorkers = " << numWorkers << ", group id = " << group_id;
...
public:
    OutgoingDispatcher(std::size_t id, std::size_t group_id, std::size_t io_thread_num,
                       std::vector<Socket> &sockets,
                       const std::vector<std::shared_ptr<Worker>> &workers,
                       LockfreeQueue<Message *> &coordinator_queue, std::atomic<bool> &stopFlag)
        : id(id),
    ...
    void start() {
        ...
        LOG(INFO) << "Outgoing Dispatcher started, numCoordinators = " << numCoordinators
                  << ", numWorkers = " << numWorkers << ", group id = " << group_id;
