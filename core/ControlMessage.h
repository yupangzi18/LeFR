...
enum class ControlMessage { STATISTICS, SIGNAL, ACK, STOP, TRANSFER, NFIELDS };
...
public:
    ...
    static std::size_t new_transfer_message(Message &message, std::size_t partition_id,
                                            std::size_t coordinator_id) {
        /*
         * The structure of a stop message: (partition id, new master id)
         */

        auto message_size =
            MessagePiece::get_header_size() + sizeof(std::size_t) + sizeof(std::size_t);
        auto message_piece_header = MessagePiece::construct_message_piece_header(
            static_cast<uint32_t>(ControlMessage::TRANSFER), message_size, 0, 0);
        Encoder encoder(message.data);
        encoder << message_piece_header;
