...
public:
    ...
    virtual void *deserialize_value(Decoder &dec, const void *key) = 0;

    virtual void serialize_value(Encoder &enc, const void *value) = 0;
    
    virtual std::string value_to_string(void *value) = 0;
...
public:
    ...
    void *deserialize_value(Decoder &dec, const void *key) override {
        std::size_t size = dec.size();
        const auto &k = *static_cast<const KeyType *>(key);
        auto &row = storage_[k];
        auto &v = std::get<1>(row);
    ...
    std::string value_to_string(void *value) {
        return std::string(static_cast<char *>(value), ClassOf<ValueType>::size());
    }
...
private:
    ...
    HashMap<N, KeyType, std::tuple<MetaDataType, ValueType>> storage_;
    std::size_t tableID_;
